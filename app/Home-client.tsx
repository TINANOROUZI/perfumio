...
export default function StoreClient() {
  ...
  const filtered = useMemo(() => {
    return PRODUCTS.filter((p) => {
      if (p.price == null) return false; // guard null price

      return (
        (cat === "All" ? true : p.category === cat) &&
        (note === "All" ? true : p.notes?.includes(note) ?? false) &&
        p.price >= price[0] &&
        p.price <= price[1] &&
        (query
          ? (p.name + " " + p.brand)
      ...
      <section className="container-a pt-10 pb-8 grid md:grid-cols-12 gap-6 items-center">
        <div className="md:col-span-7">
          ...
          <p className="mt-4 text-white/70 max-w-xl">
            Editorial curation of designer & niche fragrances plus modern
            cosmetics. Limited drops, timeless icons, and trusted essentials.
          </p>
          <div className="mt-6 flex gap-3">
            <a href="#new" className="btn-primary">
              Shop new in
            </a>
            <a href="#grid" className="btn-ghost">
              Browse all
            </a>
          </div>
        </div>
        ...
        <div className="grid lg:grid-cols-[280px,1fr] gap-6">
          ...
          <aside className="hidden lg:block p-4 rounded-2xl glass h-fit">
            ...
            <select
              ...
              className="mt-1 mb-4 w-full rounded-lg bg-ink ring-soft px-3 py-2 text-sm"
            >
